 ...
 *
 * This is the second-account counterpart to telegramUserbot.js's resolveChat, kept
 * entirely separate (own cache table, own client) so the two accounts never share state.
 */
...
// Separate from the bot (TELEGRAM_BOT_TOKEN) and from the first userbot account
// (telegramUserbot.js): this logs in as a second, distinct Telegram account (via
// scripts/telegram-userbot2-login.js) so forwarding can continue into channels the
// first account has since been banned from, as long as this account is already a
// member there.
...
async function stop() {
  if (!clientPromise2) return;
  const client = await clientPromise2.catch(() => null);
  clientPromise2 = null;

...
// Separate from the bot (TELEGRAM_BOT_TOKEN) and from userbot #1: this logs in as a
// second real Telegram account (via scripts/telegram-userbot2-login-qr.js) so posts can
// be spread across two accounts or failed over to this one if the first gets banned.
...
// to redo per forward. Resolves to the same client on every call once connected. Kept
// entirely separate from userbot #1's clientPromise so the two accounts never share state.
